import { Client, AccountBalanceQuery, TransferTransaction, PrivateKey } from "@hashgraph/sdk";
...
dotenv.config();
  ...
  async function queryBalance(accountId) {
    ...
    const accountBalanace = new AccountBalanceQuery().setAccountId(
        accountId
    );
  ...
  async function transferHbar(amount) {
    ...
    const secondAccountId = process.env.OTHER_ACCOUNT_ID;
